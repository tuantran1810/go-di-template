...
func TestUsers_Create(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			gotUser, gotAttributes, err := u.Create(context.TODO(), tt.user, tt.attributes)
			if (err != nil) != tt.wantErr {
				t.Errorf("Users.Create() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !reflect.DeepEqual(gotUser, tt.wantUser) {
				t.Errorf("Users.Create() gotUser = %v, want %v", gotUser, tt.user)
			}
			if !reflect.DeepEqual(gotAttributes, tt.wantAttributes) {
				t.Errorf("Users.Create() gotAttributes = %v, want %v", gotAttributes, tt.wantAttributes)
			}
		})
	}
}
